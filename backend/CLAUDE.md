...
**Wired up for the comments list, the first cached endpoint — the pattern to repeat for the next one:** `CommentService::getPaginated()` (highly parameterized by `sort_by` × `page`, exactly the case the paragraph above says not to cache by default) uses Laravel **cache tags** rather than trying to enumerate/delete keys itself — the redis store (unlike file/database) supports them natively. Each page is cached under `comments:index:{sort_by}:{page}` inside the `comments_list` tag. Only the resolved array (`CommentResource::resolve()` output, plus pagination scalars) is cached, never the `LengthAwarePaginator` object itself — `getPaginated()` rebuilds a fresh `Illuminate\Pagination\LengthAwarePaginator` from the cached array on every call, because `RespondTrait::respondWithJson()` does an `instanceof LengthAwarePaginator` check and calls `->items()`/`->currentPage()`/etc. on it.

**`replies_count` is deliberately kept out of that tagged page cache and out of `withCount('replies')` entirely**, because replying is far more frequent than posting a new top-level comment, and a naive "flush the whole list tag on every create" would mean the list cache almost never survives long enough to be useful. Instead each top-level comment gets its own standalone, untagged, no-TTL (`Cache::forever`) counter key `comments:{id}:replies_count`. `CommentService::hydrateRepliesCount()` runs **after** `getPaginated()` pulls its page array out of `Cache::tags(['comments_list'])->remember()` — on both a hit and a miss, not just on the miss path — because the whole point is that `replies_count` must stay live even while the surrounding page array is still a valid cache hit. It batch-reads the counters for the whole page in one `Cache::many()` call, falls back to a single grouped `whereIn('parent_id', ...)->groupBy('parent_id')` count query for any misses, warms those misses back into Redis, then overwrites the `replies_count` key directly on each already-resolved array item (mutates the plain array by reference, since by this point the data has already been through `CommentResource` — there's no Eloquent model left to attach an attribute to). `CommentService::create()` branches on `parent_id`: a new top-level comment still flushes the `comments_list` tag (it changes what a page contains) and seeds its own counter to `0`; a new reply does **not** touch the tag at all — it recomputes `Comment::where('parent_id', ...)->count()` and overwrites the parent's counter key directly. Recompute-and-overwrite was chosen over `INCR` specifically so the counter self-heals from any prior drift (lost key, stale seed) instead of compounding it — an `INCR` against a wrong or missing base would just produce a new wrong number.

`CommentService::getReplies()` is simpler — no sort/pagination variance, so it's cached under a plain untagged `comments:replies:{comment_id}` key and invalidated with a direct `Cache::forget()` for that one parent when a reply is created, rather than going through the tag system.
