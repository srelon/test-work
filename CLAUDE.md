...
- `backend/` — Laravel 12 API only; also runs Laravel Reverb (`reverb` container) for real-time broadcasting
...
**Container names:** `dzencode_app`, `dzencode_nginx`, `dzencode_db`, `dzencode_redis`, `dzencode_scheduler`, `dzencode_reverb`, `dzencode_rabbitmq`, `dzencode_queue`, `dzencode_outbox`; `dzencode_phpmyadmin`/`dzencode_rabbitmq_ui`/`dzencode_frontend_site` only when their `make pma`/`make rabbitmq-ui`/`make site` profile is started; `dzencode_caddy` in production only.

**Ports:** site + API on `SITE_PORT` (`.env`, default `8880`) — API at `/api/`; Vue dev server (`make site`) — `5173`; phpMyAdmin (`make pma`) — `8080`; Reverb (WebSocket) — `6001`; MySQL — `8101`; RabbitMQ (AMQP, needed by `queue`/`outbox`, always on) — `5672`; RabbitMQ management UI (`make rabbitmq-ui`, a thin `socat` proxy in `docker-compose.override.yml` — the broker's own port isn't published so it can't be left exposed in prod, see `backend/CLAUDE.md` § Queue) — `15672`.
...
**Wired up for comments, the first real-time feature — the pattern to repeat for the next one:** an `App\Events\*` event implements `ShouldBroadcastNow` (not `ShouldBroadcast`, which would queue the broadcast onto Laravel's own queue system — nothing in this project consumes that particular queue, so a queued broadcast would silently never send; `Now` runs it synchronously instead) and defines `broadcastOn()` (a plain `new Channel(...)` — private/presence channels need auth wiring that doesn't exist yet, see below), `broadcastAs()` (the wire event name, e.g. `'comment.created'`), and `broadcastWith()` (the payload array). `CommentService::persist()` calls `CommentCreated::dispatch($comment)` — no separate Listener needed, Laravel's broadcasting integration picks up `ShouldBroadcast*` automatically. `persist()` itself now runs from inside either the RabbitMQ consumer or a Laravel-queue fallback/retry job, never the HTTP request — see `backend/CLAUDE.md` § Queue (RabbitMQ, with a Laravel-job fallback/retry path) for why comment creation is async at all and why that fallback exists; none of this changes the `ShouldBroadcastNow` reasoning above. On the frontend, `plugins/echo.ts` sets up one shared `Echo` instance (`broadcaster: 'reverb'`); a page-scoped component subscribes with `echo.channel(name).listen('.event.name', handler)` (the **leading dot is required** — it tells Echo not to prefix the event name with the `App.Events` namespace) and cleans up with `echo.leaveChannel(name)` on unmount — see `views/Home.vue`.
